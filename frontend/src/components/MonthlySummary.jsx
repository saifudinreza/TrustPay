  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 18 }} className="summary-grid">
      ...
      <StatCard
        ...
        accent={net < 0 ? T.outRose : T.goldBright} // rose jika defisit, ungu jika surplus
        Icon={ScaleIcon}
  ...
  return (
    <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, boxShadow: '0 16px 36px -28px rgba(0,0,0,0.8)', padding: '16px 18px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <span style={{ width: 24, height: 24, borderRadius: '50%', background: `${accent}26`, color: accent, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon size={14} /></span>
      </div>
      <div style={{ fontFamily: FONT.mono, fontWeight: 500, fontSize: 19, color: accent, fontFeatureSettings: "'tnum'", letterSpacing: '-0.01em' }}>{value}</div>
