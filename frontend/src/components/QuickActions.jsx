...
// OVO-style quick action row. Each icon opens a PayModal that records a KELUAR
// transaction (simulated bill payment). `Icon` is an SVG component (see icons.jsx).
...
export default function QuickActions({ onPick }) {
  return (
    <div style={{ background: '#fff', borderRadius: 18, border: '1px solid rgba(17,32,61,0.06)', boxShadow: '0 18px 40px -28px rgba(17,32,61,0.4)', padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <h3 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 500, fontSize: 16, margin: 0, color: '#11203D' }}>Aksi Cepat</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
        {SERVICES.map((s) => {
          ...
          return (
            ...
            >
              <span style={{ width: 46, height: 46, borderRadius: 14, background: isQr ? '#11203D' : 'rgba(201,138,43,0.12)', color: isQr ? '#EFF1EC' : '#C98A2B', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Glyph size={22} />
              </span>
              <span style={{ fontSize: 12, fontWeight: 600, color: '#11203D', textAlign: 'center', lineHeight: 1.2 }}>{s.label}</span>
            </button>
          )
        })}
      </div>
